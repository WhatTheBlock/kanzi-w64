...
static const char* APP_HEADER = "Kanzi 1.5 (C) 2019,  Frederic Langlet";
...
int processCommandLine(int argc, const char* argv[], map<string, string>& map)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if ((arg == "--help") || (arg == "-h")) {
            ...
            log.println("        (EX: The source is a directory and the number of jobs > 1).\n", true);
			log.println("", true);
            ...
            ss.str(string());
            ss << "        (EX: myDir" << PATH_SEPARATOR << ". => no recursion)\n";
            log.println(ss.str().c_str(), true);
            ss.str(string());
			log.println("", true);
            ...
            if (mode.compare(0, 1, "d") != 0) {
                log.println("EX. kanzi -c -i foo.txt -o none -b 4m -l 4 -v 3\n", true);
                log.println("EX. kanzi -c -i foo.txt -f -t BWT+MTFT+ZRLT -b 4m -e FPAQ -v 3 -j 4\n", true);
                log.println("EX. kanzi --compress --input=foo.txt --output=foo.knz --force", true);
                log.println("          --transform=BWT+MTFT+ZRLT --block=4m --entropy=FPAQ --verbose=3 --jobs=4\n", true);
            }

            if (mode.compare(0, 1, "c") != 0) {
                log.println("EX. kanzi -d -i foo.knz -f -v 2 -j 2\n", true);
                log.println("EX. kanzi --decompress --input=foo.knz --force --verbose=2 --jobs=2\n", true);
            }
